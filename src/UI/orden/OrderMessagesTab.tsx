...
type ShownMessage = { id: string, message: string, timestamp: string, user: { email: string, name: string } }
...
const OrderMessagesTab = ({ orderData }: Props) => {

    const [loadingMessage, setLoadingMessage] = useState<ShownMessage>({ id: 'temp', message: '', timestamp: '', user: { email: '', name: '' } })
    const queryClient = useQueryClient()
    const orderID = orderData?.id || ''

    const { data: messages } = useQuery(['orderMessages', orderID], () => getOrderMessages(orderID), { initialData: [], refetchOnWindowFocus: false })
    ...
    return (
        <div className='flex flex-col mt-4'>
            <div className='m-4 border-4 shadow-lg min-h-[16rem] max-h-[20rem] overflow-y-scroll flex p-4 flex-col space-y-2'>
                {messages.map(mess => <OrderMessageItem key={mess.id} message={mess} userEmail={sessionData?.user?.email || ''} />)}
