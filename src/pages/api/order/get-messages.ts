...
const getMessages = async (req: NextApiRequest, res: NextApiResponse) => {
    try {
        ...
        res.status(200).json(messages.map(msg => ({ message: msg.mensaje, user: { email: msg.user.email, name: msg.user.name }, timestamp: msg.createdAt, id: msg.id, section: msg.seccion })));
    }
    catch (error) {
